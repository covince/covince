...
const DataProvider =
  ({ children, default_data_url, default_tiles_url }) => {
    const [dataSource, updateDataSource] = useQueryAsState({
      geojson: default_tiles_url,
      dataPath: default_data_url
    })

    const getData = async () => {
      const { data } = await axios.get(`${dataSource.dataPath}/lists.json`)
    ...
    const getTiles = async () => {
      const { data } = await axios.get(dataSource.geojson)
    ...
    return (
      React.cloneElement(children, { data: data, tiles: tiles, dataPath: dataSource.dataPath })
    )
  }
